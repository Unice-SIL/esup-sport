...
class UtilisateurTest extends TestCase
{
    ...
    public function testAddAutorisation(): void
    {
        ...
        $tmp_commandeDetail = new CommandeDetail($this->commande, 'inscription', $this->inscription);

        $commandeDetail = new CommandeDetail($this->commande, 'autorisation', $typeAutorisation, $tmp_commandeDetail);
        $commandeDetail->setAvoir($this->commande);
        $commandeDetail->setReferenceAvoir(10);

        $this->inscription->addCommandeDetail($commandeDetail);

        $this->assertFalse($this->utilisateur->getAutorisations()->contains($typeAutorisation));

        $this->utilisateur->addAutorisation($typeAutorisation);

        //dd($this->utilisateur->getCommandes()->()->getStatut());

        $this->assertTrue($this->utilisateur->getAutorisations()->contains($typeAutorisation));
    }
    ...
    {
        $this->utilisateur->addRole('ROLE_USER');
