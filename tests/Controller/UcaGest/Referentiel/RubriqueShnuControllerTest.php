...
class RubriqueShnuControllerTest extends WebTestCase
{
    ...
    public function testSupprimerRubriqueOrdreMin()
    {
        ...
        if (null == $rubriqueOrdreMin || $rubriqueOrdreMin->getId() == $this->rubriqueShnuId) {
            ...
            $rubriqueOrdreMin->setOrdre($r->getOrdre());
            $r->setOrdre(1);
        ...
        if (null != $rubriqueOrdreMin) {
            for ($indexOrdre = 1; $indexOrdre < $rubriqueOrdreMin->getOrdre(); ++$indexOrdre) {
                $r = $rubriqueRepo->findOneByOrdre($indexOrdre);
                $r->setOrdre($r->getOrdre() + 1);
                $em->persist($r);
            }
            $rubriqueOrdreMin->setOrdre(1);
