...
class GestionnairePanierControllerTest extends WebTestCase
{
    ...
    protected function tearDown(): void
    {
        ...
        $this->em->remove($this->em->getRepository(Utilisateur::class)->findOneByEmail('user_admin@test.fr'));
        $this->em->remove($this->em->getRepository(Utilisateur::class)->findOneByEmail('user_gestionnaire@test.fr'));
        $this->em->remove($this->em->getRepository(Utilisateur::class)->findOneByEmail('user_admin_gestionnaire@test.fr'));

        $this->em->remove($this->em->getRepository(Groupe::class)->findOneByLibelle('test_controleur_goupe_user_non_admin'));
        $this->em->remove($this->em->getRepository(Groupe::class)->findOneByLibelle('test_controleur_goupe_user_admin'));
        $this->em->remove($this->em->getRepository(Groupe::class)->findOneByLibelle('test_controleur_goupe_user_gestionnaire_panier'));
    ...
    public function testAccesRoutes($userEmail, $method, $routeName, $httpResponse, $urlParameters = [], $body = [], $ajax = false): void
    {
        ...
        $route = str_replace('id_commande_detail', $this->commandeDetail2Id, $route);
        $route = str_replace('id_commande', $this->commandeId, $route);
    ...
    */
    ...
    {
        $route = $this->router->generate('UcaWeb_ArticleSupprimer', ['id'=>$this->commandeDetail2Id]);
        $user = $this->em->getRepository(Utilisateur::class)->findOneByEmail('user_gestionnaire@test.fr');
        $this->client->loginUser($user);
        $this->client->request('GET', $route);
        $expectedRedirection = $this->router->generate('UcaWeb_CommandeEnAttenteVoir', ['id'=>$this->commandeId]);
    ...
    {
        $route = $this->router->generate('UcaWeb_CommandeEnAttenteSupprimer', ['id'=>$this->commandeId]);
        $user = $this->em->getRepository(Utilisateur::class)->findOneByEmail('user_gestionnaire@test.fr');
        $this->client->loginUser($user);
        $this->client->request('GET', $route);
