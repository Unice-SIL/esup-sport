...
use App\Form\BasculeAnneeUniversitaireType;
 ...
 */
class BasculeController extends AbstractController
{
    /**
     ...
     */
    public function afficherAction(Request $request, FlashBag $flashBag, EntityManagerInterface $em)
    {
        $listeActivites = [];
        ...
        $form = $this->createForm(BasculeType::class, null, ['liste_activites' => $listeActivites]);
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid() && $request->isMethod('POST')) {
            $this->basculeAction($form->getData()['activites'], $form->getData()['nouvelleDateDebutInscription'], $form->getData()['nouvelleDateFinInscription'], $form->getData()['nouvelleDateDebutEffective'], $form->getData()['nouvelleDateFinEffective'], $em);

            $flashBag->addMessageFlashBag('bascule.success', 'success');

            return $this->redirect($request->getUri());
        }
        $twigConfig['form'] = $form->createView();

        return $this->render('UcaBundle/UcaGest/Outils/Bascule/Bascule.html.twig', $twigConfig);
    }
     ...
     */
    public function voirAction(Request $request, FlashBag $flashBag, EntityManagerInterface $em)
    {
        $form = $this->createForm(BasculeAnneeUniversitaireType::class, ['em' => $em]);
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid() && $request->isMethod('POST')) {
            $messageFlashbag = $this->basculeAnneeUniversitaireAction($form->getData());
            $flashBag->addMessageFlashBag($messageFlashbag[0], $messageFlashbag[1]);
        ...
        return $this->render('UcaBundle/UcaGest/Outils/BasculeAnneeUniversitaire/Voir.html.twig', $twigConfig);
    }
     ...
     */
    private function basculeAction($listeActiviteId, $nouvelleDateDebutInscription, $nouvelleDateFinInscription, $nouvelleDateDebutEffective, $nouvelleDateFinEffective, EntityManagerInterface $em)
    {
    ...
    private function basculeAnneeUniversitaireAction($data)
    {
        ...
        if ($data['basculeCarteEtCotisation']) {
            ...
            //Cartes
            $commandeDetails = $em->getRepository(CommandeDetail::class)->findCommandeDetailAncienneCarte();
