...
class RessourceController extends AbstractController
{
    ...
    /**
     * @Route("/Duplicate/{id}/{typeperiode}/{periodeDebut}/{periodeFin}", name="UcaGest_RessourceDuplicate")
     * @Isgranted("ROLE_GESTION_RESSOURCE_ECRITURE")
     *
     * @param mixed $typeperiode
     * @param mixed $periodeDebut
     * @param mixed $periodeFin
     */
    public function duplicateAction(Ressource $item, $typeperiode, $periodeDebut, $periodeFin, ReservabiliteRepository $reservabiliteRepository, EntityManagerInterface $em)
    {
        $dateDebut = new DateTime($periodeDebut);
        $dateFin = new DateTime($periodeFin);

        // Ajustement de la periode pour le cas mois
        if ('month' == $typeperiode) {
            if ('01' != substr($periodeDebut, 0, 2)) {
                $dateDebut->modify('first day of next month');
            }
            $dateFin = clone $dateDebut;
            $dateFin->modify('first day of next month');
        }

        $intervalleDiff = null;
        if ('day' == $typeperiode) {
            $intervalleDiff = DateInterval::createFromDateString('1 day');
        } elseif ('week' == $typeperiode) {
            $intervalleDiff = DateInterval::createFromDateString('7 day');
        } elseif ('month' == $typeperiode) {
            $intervalleDiff = DateInterval::createFromDateString('1 month');
        }
        ...
        return $this->redirectToRoute('UcaGest_RessourceVoir', ['id' => $item->getId()]);
    }
}
