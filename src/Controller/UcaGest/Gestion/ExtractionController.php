...
class ExtractionController extends AbstractController
{
    ...
    public function listerAction(Request $request, EntityManagerInterface $em)
    {
        if ($this->isGranted('ROLE_GESTION_EXTRACTION')) {
            ...
            return $this->render('UcaBundle/UcaGest/Reporting/Extraction/Voir.html.twig', $twigConfig);
        }
     ...
     */
    public function extractListeEncadrantsAction(Request $request, EntityManagerInterface $em)
    {
     ...
     */
    public function extractionPersonnaliseeApi(Request $request, TranslatorInterface $translator, EntityManagerInterface $em)
    {
        $titleColumn = [];
        $alphabet = range('A', 'Z');
        ...
        if (!empty($lignes)) {
            foreach ($lignes as $ligne) {
                ...
                isset($inscriptionValue) ? array_push($tab, $this->getStatutInscription($inscriptionValue, $translator)) : null;
    ...
    private function getStatutInscription($inscriptions, TranslatorInterface $translator)
    {
        $result = '';
