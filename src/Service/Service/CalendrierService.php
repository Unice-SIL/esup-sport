...
class CalendrierService
{
    ...
    public function __construct(
        ...
        $this->dhtmlxEvenementRepository = $dhtmlxEvenementRepository;
    ...
    public function createPlanning(array $parametreData): JsonResponse
    {
        ...
        if (null !== $item) {
            ...
            $res = [];
            $this->logger->error('CalendrierService : Item not found, itemId = '.$parametreData['itemId']);
        }
    ...
    public function createMonthPlanning(FormatActivite $item, array $parametreData, array $twigConfig): string
    {
        ...
        return $this->twig->render('UcaBundle/UcaWeb/Activite/Calendrier/FormatActivite.calendrier.html.twig', $twigConfig);
    }
        ...
        foreach ($listeEvenements as $evenement) {
            if ('FormatAvecCreneau' == $parametreData['typeFormat']) {
                ...
                if ($evenement->getSerie() && $evenement->getSerie()->getReservabilite() && $ressource = $evenement->getSerie()->getReservabilite()->getRessource()) {
                    $campus = $ressource->getEtablissement() ?? 'Ressource';
            ...
            if (false !== $indexLigneCorrespondantCampus && false !== $indexColonneCorrespondantDate) {
    ...
    public function createDayPlanning(FormatActivite $item, array $parametreData, array $twigConfig): string
    {
        ...
        return $this->twig->render('UcaBundle/UcaWeb/Activite/Calendrier/FormatActivite.calendrier.mobile.html.twig', $twigConfig);
    }
    ...
    private function getEvents(array $parametreData, DateTime $dateDebut, DateTime $dateFin, FormatActivite $item): array
    {
        ...
        if ('FormatAvecReservation' == $parametreData['typeFormat']) {
            return array_merge(
                $this->dhtmlxEvenementRepository->findEventByRessourceAndDate($parametreData['idRessource'], $dateDebut, $dateFin),
                $this->dhtmlxEvenementRepository->findEventOfSerieByRessourceAndDate($parametreData['idRessource'], $dateDebut, $dateFin)
            );
        }
    }
}
