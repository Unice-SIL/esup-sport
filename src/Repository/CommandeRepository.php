...
class CommandeRepository extends ServiceEntityRepository
{
        ...
        $criteria->where(
            $eb->orX(
                $eb->andX(
                    ...
                    $eb->orX(
                        $eb->eq('inscriptionAvecPartenaires', null),
                        $eb->eq('inscriptionAvecPartenaires', false)
                    )
                ),
                $eb->andX(
                    $eb->eq('statut', 'apayer'),
    ...
    public function findOneByInscription($inscriptionId) {
        return $this->createQueryBuilder('c')
            ->leftJoin('c.commandeDetails', 'cd')
            ->leftJoin('cd.inscription', 'i')
            ->andWhere('i.id = :inscriptionId')
    ...
    public function findAssociatedCommandesPartenaireByInscription($inscriptionId) {
        return $this->createQueryBuilder('c')
            ->leftJoin('c.commandeDetails', 'cd')
            ->leftJoin('cd.inscription', 'i')
            ->andWhere('i.estPartenaire = :inscriptionId')
            ->setParameter('inscriptionId', $inscriptionId)
            ->getQuery()
            ->getResult()
        ;
